...
class Json:
    ...
    def loadData(self):
        try:
            ...
            # Old versions stored users globally. There is no reliable way
            # to determine which guild those old counts belonged to, so do
            # not silently copy them into every server.
            if systemData["users"] and all(
                isinstance(value, dict) and "boosts" in value
                for value in systemData["users"].values()
            ):
                systemData["users"] = {}

            for server in systemData["servers"].values():
                if isinstance(server, dict):
                    server.setdefault("bonus", 0)
                    server.setdefault("manual_users", {})
...
# Required by /setup because setup reads normal message replies.
...
def get_user_boosts(user_id, guild_id=None) -> int:
    user_id = str(user_id)
    ...
    guild_users = systemData["users"].setdefault(guild_id, {})
    ...
    return guild_users[user_id]["boosts"]


def set_user_boosts(user_id, amount: int, guild_id=None):
    user_id = str(user_id)
    ...
    guild_users[user_id]["boosts"] = max(0, amount)
...
def ensure_server_entry(guild: discord.Guild):
    ...
    if is_new:
        ...
        systemData["servers"][guild_id].setdefault("manual_users", {})
...
def format_embed_text(text: str, member: discord.Member) -> str:
    ...
    user_boosts = get_user_boosts(member.id, guild.id)
...
async def on_member_update(
    ...
    try:
        ...
        if started_boosting:
            ...
            set_user_boosts(
                after.id,
                get_user_boosts(after.id, after.guild.id) + 1,
                after.guild.id,
            )
        ...
        # --------------------------------------------------------
            ...
            # Remove one real boost from this user's tracked count.
            current_user_boosts = get_user_boosts(after.id, after.guild.id)
            set_user_boosts(
                after.id,
                max(0, current_user_boosts - 1),
                after.guild.id,
            )

            database.saveData()

            # Only remove the role if the member has no tracked boosts left.
            if (
                role is not None
                and role in after.roles
                and get_user_boosts(after.id, after.guild.id) <= 0
            ):
                try:
...
async def add_boost(
    ...
    manual_users = entry.setdefault("manual_users", {})
    manual_users[str(user.id)] = (
        manual_users.get(str(user.id), 0) + amount
    )

    new_user_total = (
        get_user_boosts(user.id, interaction.guild.id) + amount
    )
...
async def remove_boost(
    ...
    manual_users = entry.setdefault("manual_users", {})
    manual_user_boosts = manual_users.get(str(user.id), 0)
    ...
    # Only remove boosts manually credited to this user in this server.
    removable = min(
        amount,
        user_boosts,
        manual_user_boosts,
    ...
    manual_users[str(user.id)] = max(
        0,
        manual_user_boosts - removable,
    )

    if manual_users[str(user.id)] == 0:
        manual_users.pop(str(user.id), None)
    ...
    if (
        reward_role_id
        and get_user_boosts(user.id, interaction.guild.id) <= 0
